...
func animate_halos():
	...
	tween.tween_property(halo1, "modulate:a", 0.8, 0.7).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property(halo1, "scale", Vector2(1.1, 1.1), 0.7).set_trans(Tween.TRANS_CIRC)
	
	# Halo2 animation (outer halo)
	tween.parallel().tween_property(halo2, "modulate:a", 0.8, 0.7).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property(halo2, "scale", Vector2(1.25, 1.25), 0.7).set_trans(Tween.TRANS_CIRC)

	# Contract both halos
	tween.chain().tween_property(halo1, "modulate:a", 0.1, 0.6).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property(halo1, "scale", Vector2(1, 1), 0.6).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property(halo2, "modulate:a", 0.1, 0.6).set_trans(Tween.TRANS_CIRC)
	tween.parallel().tween_property(halo2, "scale", Vector2(1, 1), 0.6).set_trans(Tween.TRANS_CIRC)
